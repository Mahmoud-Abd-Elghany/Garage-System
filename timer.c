...
volatile unsigned char time_in_sec = 0;
volatile unsigned char current_time = 0;
void (*callback_function)(void) = (void*)0;

ISR(TIMER0_OVF_vect){
	counter += 1;
	current_time = counter*0.262;
	if(current_time > time_in_sec){
		callback_function();
		TCCR0 = 0;
		CLR_BIT(TIMSK,TOIE0);
		counter = 0;
...
void timer_OVF_interrupt_Callback(void (*callback_func)(void),unsigned char time_sec){
	/* select Overflow mode with Prescaler 1024*/
	SET_BIT(TCCR0,CS00);
	SET_BIT(TCCR0,CS02);
	/* enable interrupt*/
	sei();
